...
| Path | Purpose |
|---|---|
| [`New-BulkADUsers.ps1`](New-BulkADUsers.ps1) | The bulk-creation script |
| [`users-template.csv`](users-template.csv) | CSV template matching the script's expected columns |
| `01-support-ticket.png` – `11-ticket-solved.png` | Screenshots captured from the resolution, referenced throughout this README |
...
![Support ticket requesting bulk user creation](01-support-ticket.png)
...
![Blank Google Sheet with column headers](02-sheet-setup.png)
...
![Username generated with a formula](03-username-formula.png)
...
![Fully populated sheet with OU distinguished names](04-sheet-populated-with-ou.png)
...
![CSV exported to the Downloads folder](05-export-csv-download.png)

### 4. Review the PowerShell script

The script ([`New-BulkADUsers.ps1`](New-BulkADUsers.ps1)) imports the CSV, builds a UPN and display name per row, checks whether the account already exists, and creates it with `New-ADUser`.

![PowerShell script contents](06-powershell-script.png)
...
![Script running in the PowerShell console](07-script-execution.png)

Each successfully created account is printed to the console in green:

![Console output confirming each user was created](08-successful-output.png)
...
![New user visible in the correct OU in ADUC](09-ad-verification-hr-ou.png)
...
![Reply to the ticket confirming completion](10-ticket-resolved-reply.png)

Mark the ticket solved once the requester confirms there's nothing else needed.

![Ticket marked as solved](11-ticket-solved.png)
